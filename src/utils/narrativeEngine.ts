...
export const getCoachVerdict = (userWon: boolean, coachIQ: number): string => {
  const verdicts = coachIQ >= 75 ? highIQVerdicts : lowIQVerdicts;
  return getRandom(userWon ? verdicts.win : verdicts.loss);
};
...
export const getNarrative = (params: NarrativeParams): GameNarrative => {
  ...
  const coachVerdict = getCoachVerdict(userWon, coachIQ);
